...
const voucherService = {
  ...
  async createVoucher(data) {
    const response = await adminHttpRequest.post("/vouchers", data);
    return response.data;
  },

  // READ - Lấy danh sách vouchers với phân trang
  async getAllVouchers(params = {}) {
    const response = await adminHttpRequest.get("/vouchers", { params });
    return response.data;
  },

  // READ - Lấy voucher theo ID
  async getVoucherById(id) {
    const response = await adminHttpRequest.get(`/vouchers/${id}`);
    return response.data;
  },

  // READ - Lấy voucher theo mã
  async getVoucherByCode(code) {
    const response = await adminHttpRequest.get(`/vouchers/code/${code}`);
    return response.data;
  },

  // UPDATE - Cập nhật voucher
  async updateVoucher(id, data) {
    const response = await adminHttpRequest.put(`/vouchers/${id}`, data);
    return response.data;
  },

  // DELETE - Xóa voucher
  async deleteVoucher(id) {
    const response = await adminHttpRequest.del(`/vouchers/${id}`);
    return response.data;
  },

  // UPDATE STATUS - Cập nhật trạng thái voucher
  async updateVoucherStatus(id, status) {
    const response = await adminHttpRequest.patch(`/vouchers/${id}/status`, {
      status,
    });
    return response.data;
  },
  ...
  async validateVoucher(code, userId, orderData) {
    const response = await adminHttpRequest.post(`/vouchers/validate/${code}`, {
      user_id: userId,
